          ...
          {diags.length === 0 ? (
            ...
            <>
              {diags.slice(0, 10).map((d, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", gap: 2, borderBottom: "1px solid #2a2a2a", paddingBottom: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                    <span style={{ color: "#777", fontSize: 11, wordBreak: "break-word" }}>{d.name}</span>
                    <span style={{ color: diagColor(d.level), fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{diagLabel(d.level)}</span>
                  </div>
                  {d.message && <span style={{ color: "#555", fontSize: 10 }}>{d.message}</span>}
                </div>
              ))}
