# Testing Procedure for umdloop-open-monitors.sh
...
`umdloop-open-monitors.sh` if they differ from `(0 1920 3840)`.
...
```bash
chmod +x umdloop-open-monitors.sh
```
...
# Copy the script
mkdir -p ~/.local/bin
cp umdloop-open-monitors.sh ~/.local/bin/umdloop-open-monitors.sh
chmod +x ~/.local/bin/umdloop-open-monitors.sh

# Copy the .desktop file
mkdir -p ~/.config/autostart
cp umdloop-open-monitors.desktop ~/.config/autostart/umdloop-open-monitors.desktop
```

2. Update the `Exec=` path in the `.desktop` file if your username isn't `cc2`:

```bash
sed -i "s|/home/cc2|$HOME|" ~/.config/autostart/umdloop-open-monitors.desktop
```
...
rm ~/.config/autostart/umdloop-open-monitors.desktop

# Remove the script
rm ~/.local/bin/umdloop-open-monitors.sh
